...
class DashboardKepalaSekolah(ctk.CTk):
    ...
    def show_dashboard(self):
        ...
        try:
            ...
            for index, row in enumerate(rows):
                ...
                values = (row[0], prob_text, row[2], row[3])
    ...
    def show_laporan(self):
        ...
        table.column("nama", width=280)
        table.column("probabilitas", width=150, anchor="center")
        table.column("hasil", width=180, anchor="center")
        table.column("tanggal", width=150, anchor="center")
        ...
        try:
            ...
            for index, row in enumerate(rows):
                ...
                prob = row[2]
                prob_text = f"{float(prob):.1f}%" if prob is not None else "0%"
                values = (row[0], row[1], prob_text, row[3], row[4])
                table.insert("", "end", values=values, tags=(tag,))
    ...
    def download_laporan(self):
        try:
            ...
            # Header Laporan
            judul = f"LAPORAN HASIL PREDIKSI SISWA\nSDN KLEGEN\n{datetime.now().strftime('%d %B %Y')}"
            ...
            # Build PDF
            elements = []
            elements.append(Paragraph(judul, judul_style))
